    ...
    private static void receiveReplicaTaskAward(String taskType, String taskTitle) {
        try {
            ...
            if (MessageUtil.checkSuccess(TAG, jo)) {
                ...
                Log.forest("神奇海洋🐳领取[" + taskTitle + "]奖励[潘多拉能量*" + incAwardCount + "]");
            }
        } catch (Throwable t) {
        ...
        try {
            ...
            for (int i = 0; i < jaTaskList.length(); i++) {
                ...
                if (MessageUtil.checkSuccess(TAG, jo)) {
                    ...
                    Log.forest("海洋任务🎖️领取[" + taskTitle + "]奖励[" + awardCount + "块拼图]");
                }
            }
        } catch (Throwable t) {
    ...
    private static Boolean exchangeUniversalPiece(int number) {
        try {
            ...
            if (MessageUtil.checkResultCode(TAG, jo)) {
                ...
                Log.forest("神奇海洋🐳制作[万能拼图]" + exchangeNum + "张,剩余" + duplicatePieceNum + "张重复拼图");
                return true;
            }
        } catch (Throwable t) {
            Log.i(TAG, "exchangeUniversalPiece error:");
    ...
    private static Boolean useUniversalPiece(JSONArray assetsDetails, String name, int holdsNum) {
        try {
            ...
            if (MessageUtil.checkResultCode(TAG, jo)) {
                ...
                Log.forest("神奇海洋🐳使用[万能拼图]" + userCount + "张,获得[" + name + "],剩余" + holdsNum + "张");
                return true;
            }
        } catch (Throwable t) {
            Log.i(TAG, "useUniversalPiece error:");
